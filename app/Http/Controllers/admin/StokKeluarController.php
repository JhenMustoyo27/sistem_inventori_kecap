...
class StokKeluarController extends Controller
{
    ...
    /**
     * Menampilkan form untuk mengedit data stok keluar.
     *
    ...
    public function edit(StokKeluar $stokKeluar)
    {
        ...
        return view('admin.stok_keluar.edit', compact('stokKeluar', 'availableKecapMasuk'));
    }

    /**
     * Memperbarui data stok keluar yang sudah ada.
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  \App\Models\StokKeluar  $stokKeluar
     * @return \Illuminate\Http\RedirectResponse
    ...
    public function update(Request $request, StokKeluar $stokKeluar)
    {
        ...
        try {
            $oldQuantity = $stokKeluar->jumlah_keluar;
            $newQuantity = $request->jumlah_keluar;

            $masterStok = $stokKeluar->masterStok; // Get the associated MasterStok batch

            if (!$masterStok) {
                DB::rollBack();
                return redirect()->back()->withErrors(['error' => 'Batch stok masuk terkait tidak ditemukan.']);
            }
            ...
            if ($quantityDifference > 0) {
                $availableInBatch = $masterStok->stok_terakhir;
                if ($availableInBatch < $quantityDifference) {
                    DB::rollBack();
                    return redirect()->back()->withErrors(['jumlah_keluar' => "Stok tersedia di batch ini tidak mencukupi untuk menambah jumlah keluar. Stok tersedia: {$availableInBatch}."]);
                }
            }

            // Adjust the master stock (subtract if quantity increased, add if quantity decreased)
            $masterStok->stok_terakhir -= $quantityDifference;
            $masterStok->save();

            // Update the StokKeluar record
            $stokKeluar->update([
                'jumlah_keluar' => $newQuantity,
                'tanggal_keluar' => $request->tanggal_keluar,
                'harga_jual' => $request->harga_jual,
                'keterangan' => $request->keterangan,
            ]);
            ...
            StokHistory::create([
                'kecap_masuk_id' => $stokKeluar->kecap_masuk_id,
                'master_stok_id' => $stokKeluar->master_stok_id,
                'event_type' => 'stok_keluar_updated',
                'kode_kecap' => $stokKeluar->kecapMasuk->kode_kecap,
                'ukuran' => $stokKeluar->kecapMasuk->ukuran,
                'quantity' => -$quantityDifference, // Log the change in quantity (negative for deduction, positive for return)
                'current_stock_after_event' => $masterStok->stok_terakhir,
                'description' => "Data stok keluar diperbarui. Perubahan jumlah: " . ($quantityDifference > 0 ? '+' : '') . $quantityDifference . " botol.",
            ]);

            DB::commit();

            return redirect()->route('admin.stok_keluar.index')->with('success', 'Data stok keluar berhasil diperbarui!');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Error updating stok keluar: " . $e->getMessage(), ['exception' => $e]);
            return redirect()->back()->withErrors(['error' => 'Terjadi kesalahan saat memperbarui stok keluar. Silakan coba lagi.']);
        }
    }
    ...
    public function destroy(StokKeluar $stokKeluar)
    {
        ...
        try {
            $masterStok = $stokKeluar->masterStok;

            if (!$masterStok) {
                DB::rollBack();
                return redirect()->back()->with('error', 'Batch stok masuk terkait tidak ditemukan untuk data yang akan dihapus.');
            }

            // Return the quantity back to the master stock
            $masterStok->stok_terakhir += $stokKeluar->jumlah_keluar;
            $masterStok->save();
            ...
            StokHistory::create([
                'kecap_masuk_id' => $stokKeluar->kecap_masuk_id,
                'master_stok_id' => $stokKeluar->master_stok_id,
                'event_type' => 'stok_keluar_deleted',
                'kode_kecap' => $stokKeluar->kecapMasuk->kode_kecap,
                'ukuran' => $stokKeluar->kecapMasuk->ukuran,
                'quantity' => $stokKeluar->jumlah_keluar, // Quantity returned (positive)
                'current_stock_after_event' => $masterStok->stok_terakhir,
                'description' => 'Data stok keluar dihapus dan stok dikembalikan. Kode: ' . $stokKeluar->kecapMasuk->kode_kecap . ', Ukuran: ' . $stokKeluar->kecapMasuk->ukuran . ', Jumlah: ' . $stokKeluar->jumlah_keluar . '.',
            ]);
            ...
            return redirect()->route('admin.stok_keluar.index')->with('success', 'Data stok keluar berhasil dihapus dan stok dikembalikan!');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Error deleting stok keluar: " . $e->getMessage(), ['exception' => $e]);
            return redirect()->back()->with('error', 'Terjadi kesalahan saat menghapus stok keluar. Silakan coba lagi.');
        }
    }

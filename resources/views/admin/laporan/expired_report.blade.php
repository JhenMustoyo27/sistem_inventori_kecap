...
<!-- Filter dan Tombol Download -->
        <section class="bg-white p-6 rounded-xl shadow-md mb-8">
                ...
                <form action="{{ route('admin.laporan.expired') }}" method="GET" class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.laporan.expired') }}" class="bg-gray-400 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition duration-300 ease-in-out w-full sm:w-auto">
                            Reset
                        </a>
                    @endif

...
@section('title', 'Laporan Stok Expired - Admin')

@section('content')
<!-- Filter dan Tombol Download -->
        <section class="bg-white p-6 rounded-xl shadow-md mb-8">
                <h2 class="text-2xl font-semibold text-gray-800 mb-4">Filter Laporan Expired</h2>
                <form action="{{ route('admin.laporan.expired') }}" method="GET" class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.laporan.expired') }}" class="bg-gray-400 text-center hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition duration-300 ease-in-out w-full sm:w-auto">
                            Reset
                        </a>
                    @endif
                    <a href="{{ route('admin.laporan.download_expired_pdf', ['search' => request('search')]) }}" class="bg-green-600 text-center hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-300 ease-in-out w-full sm:w-auto">
                        Unduh PDF
                    </a>
                </form>
            </section>
                    ...
                    <tbody class="text-gray-700 text-sm font-light">
                        @forelse ($laporanExpiredPaginated as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-6 text-left">{{ $item['tanggal_expired'] ? $item['tanggal_expired']->format('d-m-Y') : 'N/A' }}</td>
